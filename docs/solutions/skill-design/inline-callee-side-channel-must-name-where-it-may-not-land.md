...
Skills in this plugin run inline in the caller's context: oh-my-pi loads an invoked skill's body into the same context, as upstream's hosts do. There is no subagent boundary between caller and callee.
...
This was observed upstream on 2026-09-13 in Claude Code; the fork runs skills inline the same way, so the path applies here unchanged. An `lfg` run routed a judgment request through `ce-pov`, which writes its chat block through `ce-noslop` in edit mode. The user-facing message opened with a sentence describing the edit pass ("Edit-mode pass applied: split two long sentences, replaced ... labels with plain sentences, ...") before the verdict. That sentence is process narration about the agent's own drafting, which `ce-noslop` itself flags at `skills/ce-noslop/references/patterns.md:59` (rule 39, "Process narration in a report"). It also leaks internal vocabulary ("edit-mode pass") that the user never asked about.

On this fork, no sibling skill references `ce-noslop` yet: the whole-skill port lands it unwired, while upstream at the same date had eleven sibling skills referencing it across sixteen files (grep `ce-noslop` under `skills/*/SKILL.md` and `skills/*/references/*.md`). Any caller that loads the skill inline — a user, a sibling, or a pipeline step — faces the same channel, and none of upstream's consumers said what to do with the change line. Teaching each consumer to strip it would be a patch in every consumer for one callee defect, and a consumer added later would inherit the gap silently.
...
Evaluation for this kind of change: fresh-agent cells via `bun run test:skill-eval-cell`. Upstream graded pre (HEAD) and post (worktree) A/B on Claude and Codex: the edit-then-print cell showed pre-change Claude putting "Changed: split the run-on into single-idea sentences, ..." inside the user message markers, and pre-change Codex printing the summary after the markers but still in its output; post-change both hosts printed only the verdict with every source fact intact. The detect-mode cell listed findings on both hosts pre and post, as intended. The fork's ported cells (`ce-noslop/facts-survive-the-edit` and friends) run post-only on omp and pin the same contract: the summary the caller asked for stays outside the RESULT block.
...
The failure, as it reached the user from an upstream `lfg` run through `ce-pov`:
...
- `invocation-opt-out-flags-block-sibling-skill-invocation.md` (upstream): the same fact (a sibling reaches `ce-noslop` by reading it into its own context) seen from the activation side.
