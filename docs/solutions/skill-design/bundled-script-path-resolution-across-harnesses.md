...
The three tiers (read-time relative, prose pointer plus "from this skill's directory" cue, `SKILL_DIR` anchor for executed shell), the load-bearing trailing `;`, the `BASH_SOURCE` rule for a script that needs its own directory, and the substituted-skill-dir warning all live in `docs/solutions/developer-experience/always-on-agents-md.md` under "Platform-variable tiers". This doc keeps the lesson behind them.
