...
Wire a native surface as a first-class versioned surface: bump its version file(s) with the root version when cutting a fork release (see `AGENTS.md` "Release versioning"), and validate with `bun run plugin:validate`; static catalog files with no version stay unversioned and are validated instead.
