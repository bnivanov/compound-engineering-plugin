...
- **Program item (ecosystem graft — no upstream SHA applies):** R15. No code shipped; the unit's honest disposition is blocked-with-reason.

| Item | Disposition | Evidence |
|---|---|---|
| R15 post-compaction skill re-injection (superpowers) | blocked-with-reason | U13 port record in `docs/omp-capability-audit.md`. Both audit preconditions hold — `session.compacting` returns `context[]` (injected into `<additional-context>`) and lazy skill loading is real (R3c) — so the gap is genuine: compaction rewrites a previously loaded skill body out of context and only metadata remains. The port is blocked on two structural blockers, not on registration safety: (1) no active-skill state convention — nothing in omp v18.1.21 records which skill is active at compaction time, so a handler could only guess or inject nothing useful; (2) no runnable firing harness — observing the re-injected `<additional-context>` block requires the live registration the flip condition is gated on. The mandatory-activation half stays unported per the 2026-09-10 eval's O01 Skip. No test: no code shipped |

### Port notes (U13)

- **The opt-in path was evaluated, not dismissed.** `.pi/extensions/compound-engineering.ts` could register `session.compacting` behind an opt-in gate without changing non-opted-in checkout behavior; U13 recorded that this answers only the "changes every session" objection, and that the two structural blockers above are what actually block the port. This is the difference between "we could not be bothered" and a recorded capability gap.
...
- **Audit rows corrected (orchestrator):** the U1 audit's R12 and R15 rows asserted the ports would be hook-enforced. Both were disproved by the ports — R12's `session_stop` cannot reach the target event, and R15's hook existence is not a usable consumer. Rows and the audit-level blocked summary now carry the port verdicts and flip conditions, so the findings record does not read green where the code says blocked.
