...
- **Consumer-side anchor guard and failure condition (2026-09-15 fork adaptation).** Upstream's six call blocks point the model at a `SKILL_DIR` it fills in, and carry a "when the command yields no JSON, packs are unresolved; say so once; never stop the run" clause; a wrong anchor therefore degraded to silence, observed live on this fork — a `ce-plan` run whose agent held the right skill path, probed for another install instead, found a stale copy without `scripts/`, and produced a plan citing zero rules with no error in any artifact. Each consumer now derives `RESOLVER` from that anchor behind an existence guard that names the path and exits 1, and states that a failed resolution is not "no packs": an unreachable resolver, or JSON carrying `errors`, is recorded where that run reports pack status (a plan's inputs, a review's `coverage.compound_packs` with `status: unresolved`), while `warnings` alone stay warn-and-continue per `docs/guides/packs.md`. `ce-code-review`'s Coverage key gains the status vocabulary `resolved` / `unresolved` / `skipped-remote`, so a review cannot read as pack-clean when nothing resolved. The resolver copies were untouched by that change (sha256 `0e7de948…` at that commit; they diverge from upstream since the exit-code departure below); no `--declared-only` pre-check was added, because the full result already carries `entries` and a second call cannot decide anything the guard does not. Pins: `CALL_BLOCK_FILES` and the Coverage-status test in `tests/skills/ce-packs.test.ts`.
- **Resolver exit code (2026-09-15 fork adaptation).** Upstream's `packs-resolve.py` prints an `errors` array and exits 0 whatever happens, so a declared pack that did not load arrives as a command that succeeded, with the failure in a field. The seven copies now exit 1 when `errors` is non-empty; `--declared-only` keeps exit 0, being a probe that answers a shape question rather than an action that failed. The digest pin in `tests/skills/ce-packs.test.ts` moves from `0e7de948…` to `34ca03b1…`, so the fork's resolver is no longer byte-identical to upstream's — a future sync must not overwrite it with upstream's copy. The module header's exit-status contract was corrected in the same departure, so upstream's promise that per-entry failures ride in `errors` at exit 0 is now false on this fork. Flip condition: upstream returns non-zero on its own errors.
