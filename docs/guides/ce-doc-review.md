...
Cross-model independence on OMP is dispatching a `reviewer` agent through the host `task` tool. When the conditional judgment trio (adversarial, product-lens, security-lens) activates, those lenses can run as explicit `reviewer` reads of their own briefs. Agreement between a reviewer return and its in-process twin can raise confidence by one level only when the peer receipt attests `independence_verified: true` and the combined evidence meets that level's definition; agreement does not make a finding important or grant permission to edit. Coherence, scope-guardian, and feasibility stay single-model so the pass does not spawn a reviewer on every review.
