...
- **Durable.** Everything else: the full unified plan file with confidence check, document review, an independent planner check, and handoff menu.
...
Document review then runs non-interactively. Safe auto-fixes apply silently; remaining findings surface as one line above the menu (`Doc review applied 2 fixes. 3 decisions, 1 FYI remain.`). An independent `planner` agent then checks the written plan; a missing planner return is recorded as unreachable, not as a pass. The menu offers: start `ce-work` (recommended), run it as a `/goal` when the host supports that, decide on remaining review items or prototype a remaining feel-question, create a tracked issue, or open the file if it is HTML. There is no Proof option on the software menu and no pause option. The file is already saved.
...
Authoring stays on the session model. After a Durable plan is written, a `planner` agent checks it. That check is not a pass if the planner cannot launch. For stronger authoring, run the session on a stronger model.
