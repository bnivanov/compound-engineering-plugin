...
Cross-model independence on OMP is dispatching a `reviewer` agent through the host `task` tool. When adversarial is selected and the working tree is the reviewed head (current branch, or a PR whose local tree already matches the PR head), that lens runs as one explicit `reviewer` read of the same adversarial brief. A started reviewer replaces the in-process `adversarial` persona; they never both receive the same brief. The in-process persona covers the lens when no explicit read was requested or the run cannot complete. Agreement between the reviewer read and another in-process reviewer promotes only when the receipt's `independence_verified` is `true` from host/backend attestation (known serving family different from the session family, never reviewer prose). Otherwise the read is attributed evidence, never separate-model corroboration.
