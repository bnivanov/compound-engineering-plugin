...
| What it produces | Software: a requirements-only unified plan in `docs/plans/` with R/A/F/AE IDs. Non-software: chat synthesis, optional save, optional Proof publish, optional handoff to `ce-plan`. Lightweight alignment can skip the doc. |
...
An empty invoke asks what to explore. A path to an existing requirements-only plan offers resume. `output:html` changes the artifact format.
