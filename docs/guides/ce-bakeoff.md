...
Bakers prefer different model families through native host access, then available authorized model CLIs. If those routes are unavailable or fail, fresh agents on the host's own model are the fallback. Explicit model choices and restrictions take precedence; planning and brainstorming strip `plan_model:` and `brainstorm_model:` tokens during intake rather than dispatching on them. Bake-off owns this dispatch with direct scoped calls and builds no new dispatch framework. It briefly discloses external recipients and read scope unless already disclosed. Diversity is preferred; fresh contexts are required.

A fresh subagent dispatched with the `ce-pov` skill is required before selection. A different model family is preferred: use native host model access when suitable, otherwise an available authorized model CLI or adapter. A fresh same-family judge is a disclosed fallback; no independent judge means incomplete. The coordinator reconciles the assessment with its own comparison and verifies consequential premises against source evidence before returning the winner. An explicitly requested oracle, or a consequential disagreement that survives source checking and warrants consultation within budget, uses `ce-pov`'s existing panel. All judging and verification fit within the Bake-off budget.
