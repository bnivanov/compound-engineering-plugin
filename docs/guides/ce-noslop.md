...
Sibling skills write their prose through `ce-noslop` at the point where they compose it, and their own presentation contracts sit on top of it. Upstream wires `ce-commit-push-pr` (PR title and body), `ce-code-review` (human-readable findings), `ce-plan` and `ce-brainstorm` (plan prose sections), `ce-promote` (the announcement drafts it writes itself; Spiral-returned drafts are presented as returned), and `ce-resolve-pr-feedback` (replies posted as the PR author) this way. This fork's sibling wiring is not landed yet: invoke `ce-noslop` directly on their output for now.
...
The skill is not in context when an agent writes an ordinary chat reply, so the invocation above covers artifacts, not the agent's own reports and summaries. A standing instruction in the project's agent-instructions file closes that gap. This fork's `ce-setup` offers the knowledge-store mention and the `ce-compound` directive; add this instruction by hand with the same placement rules: preview the exact text and location, then append only on approval, and skip it when the file already carries an instruction covering the report boundary, the invocation of the skill, and the exclusions.
...
- [`ce-commit-push-pr`](./ce-commit-push-pr.md): PR descriptions composed under this skill upstream
- [`ce-code-review`](./ce-code-review.md): findings written under this skill upstream
- [`ce-plan`](./ce-plan.md) and [`ce-brainstorm`](./ce-brainstorm.md): plan prose written under this skill upstream
- [`ce-doc-review`](./ce-doc-review.md): what a document says, not how it reads
- [`ce-setup`](./ce-setup.md): standing-instruction offers this fork covers with the knowledge-store mention and the `ce-compound` directive
