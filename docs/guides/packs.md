...
| `pack resolver not found at <path>` | The skill ran a resolver at a path that is not the install it loaded: `SKILL_DIR` pointed at another copy, and a stale global install has no `scripts/`. Re-run from the skill this session loaded; a resolver that cannot be reached is never read as "no packs declared" |
