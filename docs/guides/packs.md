...
| `errors` in the JSON, packs missing this run | A declared pack did not load — its entry is missing, malformed, or unpublished. The run continues with the packs that did load, and the lines are recorded where the run reports pack status (a plan's inputs, a review's `coverage.compound_packs` with `status: unresolved`), not mentioned once and dropped |
| `pack resolver not found at <path>` | The skill ran a resolver at a path that is not the install it loaded: `SKILL_DIR` pointed at another copy, and a stale global install has no `scripts/`. Re-run from the skill this session loaded; a resolver that cannot be reached is never read as "no packs declared" — declaredness is unknown when the resolver cannot be reached, so the run records the failure rather than assuming the repo declared none |
