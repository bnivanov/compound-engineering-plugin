...
For an external-adoption candidate, seed that researcher to record primary-source revision or pin, date, maintenance, and the license the source itself states. Popularity metrics stay discovery-only.
...
**External-adoption provenance.** When the subject is an external-adoption candidate, verified-external-facts include the candidate's primary-source revision or equivalent pin, that source's date, a maintenance signal taken from that source, and the license the source itself states. Read the candidate's own primary source. Stars, curated lists, and a recent commit are discovery evidence only; they do not establish correctness, compatibility, or permission to adopt or redistribute. An unknown or unstated license is not permission.
