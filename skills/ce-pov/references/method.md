...
An approach-set POV judges developed options supplied by the user, conversation, or calling skill. Discovering an open field belongs to `ce-ideate`, and establishing goals or criteria belongs to `ce-brainstorm`. Preserve the reasons, material tradeoffs, evidence, and conditions behind the position.
...
Still-open strategic questions belong in STRATEGY.md; recommend `ce-strategy` when the result calls for one. Other remaining questions stay in this POV or in the existing plan or requirements artifact that already holds them. Do not open issues or labels for them.
