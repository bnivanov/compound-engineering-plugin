...
### Phase 0.5: OMP Memory Scan

Before launching Phase 1 subagents, check OMP memory via recall (memory://) for notes relevant to the problem being documented.

1. Recall relevant notes from OMP memory (memory://) for the problem being documented
2. If no relevant notes are found, skip this step and proceed to Phase 1 unchanged
...
```
## Supplementary notes from OMP memory
...
5. Pass this block as additional context to the Context Analyzer and Solution Extractor task prompts in Phase 1. If any memory notes end up in the final documentation (e.g., as part of the investigation steps or root cause analysis), tag them with "(OMP memory)" so their origin is clear to future readers.
...
#### 1. **Context Analyzer**
   ...
   - Incorporates OMP memory excerpts (if provided by the orchestrator) as supplementary evidence
...
#### 2. **Solution Extractor**
   ...
   - Incorporates OMP memory excerpts (if provided by the orchestrator) as supplementary evidence -- conversation history and the verified fix take priority; if memory notes contradict the conversation, note the contradiction as cautionary context
