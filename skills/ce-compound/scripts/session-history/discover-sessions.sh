#!/usr/bin/env bash
# Discover omp session files for a repo.
#
# Usage: discover-sessions.sh <repo-name> <days> [--cwd /abs/repo/root] [--platform omp]
#
...
#   --cwd      Absolute repo root. Used for the omp raw-bucket probe.
#              extract-metadata.py --cwd-filter matches recorded cwd.
#   --platform Must be omp when set. Pi is not a discovery platform.

set -euo pipefail

REPO_NAME="${1:?Usage: discover-sessions.sh <repo-name> <days> [--cwd /abs/repo/root] [--platform omp]}"
DAYS="${2:?Usage: discover-sessions.sh <repo-name> <days> [--cwd /abs/repo/root] [--platform omp]}"
PLATFORM="all"
REPO_CWD=""
