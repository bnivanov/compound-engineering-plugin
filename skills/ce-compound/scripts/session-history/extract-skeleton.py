#!/usr/bin/env python3
"""Extract the conversation skeleton from a Pi or oh-my-pi (omp) JSONL session file.

Usage:
  cat <session.jsonl> | python3 extract-skeleton.py
  cat <session.jsonl> | python3 extract-skeleton.py --output PATH

Auto-detects platform (Pi or oh-my-pi (omp)) from the JSONL structure.
Extracts:
...
# Framework wrapper tags to strip from user message content.
...
def _safe_slice(value, n):
    ...
    Some tool inputs put structured data (dicts, lists) in fields like
    `query` or `prompt`. `dict[:N]` raises TypeError, so guard every slice
...
handlers = {"pi": handle_pi, "omp": handle_pi}
handler = handlers.get(detected, handle_pi)
