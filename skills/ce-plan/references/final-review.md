...
- Do not write a launch prompt into the doc. The launch prompt is generated at handoff (Phase 5.4 menu, OMP launch via herdr or headless `omp -p`) from the plan's current content, so it never goes stale; it points to Goal Capsule, Verification Contract, Definition of Done, and U-IDs rather than duplicating them.
