...
1. Search for Slack tools using the platform's tool discovery mechanism (e.g., tool listing or schema inspection). Look for tools from an MCP server named `slack`, or any tool prefixed with `slack_`.
