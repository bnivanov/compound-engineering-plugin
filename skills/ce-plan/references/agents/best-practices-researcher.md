...
1. **Discover Available Skills**:
   ...
   - For maximum compatibility, check project/workspace OMP plugin skill directories for workspace-local skills
   - Also check user/home OMP plugin skill directories for globally installed skills
   - OMP plugin skills may be discovered from the current working directory upward to the repository root, not only from a single fixed repo root location
   - If the current environment provides an `AGENTS.md` skill inventory, use that list as the initial discovery index, then open only the relevant `SKILL.md` files
