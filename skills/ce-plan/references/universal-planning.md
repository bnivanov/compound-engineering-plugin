...
- **Pipeline mode?** If invoked from `lfg` or any `disable-model-invocation` context: tell the user this is a non-software task, `lfg` requires the software-only `ce-work` path, and they should invoke `/skill:ce-plan` directly for non-software planning. For this user-runnable output, render `/skill:ce-plan` (the canonical OMP `/skill:` form). Render only the invocation as inline code and output one form only, then stop.
