...
Writing the file, checking confidence, running or explicitly skipping `ce-doc-review`, and completing the independent planner check (or recording `planner_check: unreachable`) are intermediate milestones. Pipeline mode is complete only when the plan, confidence check, non-interactive document-review state, and planner-check state are returned to its caller; the caller owns the next action.
