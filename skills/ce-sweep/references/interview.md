...
**User-runnable invocation rendering.** Whenever this interview prints or registers a `ce-sweep` invocation, render `/skill:ce-sweep` (plus any arguments). Render only the invocation as inline code and output one form only.
