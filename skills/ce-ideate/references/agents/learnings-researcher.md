...
Use the native file-search/glob tool (e.g., `Glob`) to discover which subdirectories actually exist under `<root>/solutions/` at invocation time. Do not assume a fixed list — subdirectory names are per-repo convention and may include any of:
...
**Use the native content-search tool (e.g., `Grep`) to find candidate files BEFORE reading any content.** Run multiple searches in parallel, case-insensitive, returning only matching file paths:
