...
**Cross-model returns.** A `<reviewer-name>-omp` return merges with its in-process twin under the same one-fix test. Whether that merge counts as *independent corroboration* is decided in 3.4 by the return's `independence_verified` flag -- not here.
...
**Cross-model returns count as independent personas here only when the return's top-level `independence_verified` is `true`.** A return with `false` or a missing flag remains useful attributed reviewer evidence, but cannot trigger anchor promotion or be described as separate-model corroboration. It still merges in 3.3 under the one-fix test; merging and corroboration are separate questions. The flag is true only when host/backend attestation supplies a known serving family different from the session family, never from reviewer prose.

When the cross-model judgment pass ran (see `references/cross-model-review.md`), each peer return enters synthesis as `<reviewer-name>-omp` (e.g. `adversarial-omp`). Agreement with its in-process twin promotes only when the peer receipt has attested `independence_verified: true`; otherwise the findings merge in 3.3 as attributed evidence without promotion. **The whole-document sweep** (`whole-doc-omp`) has **no in-process twin**. Its findings remain attributed evidence and never promote unless independently corroborated under 3.3 and the sweep receipt has attested `independence_verified: true`; a sweep alone never promotes. **Corroboration only, never apply authority:** a peer-only finding is never silently applied as `safe_auto`. Preserve the peer cap in 3.6 and routing in 3.7: peer-only `manual` findings stay `manual`, and findings otherwise eligible for Apply require confirmation as `gated_auto`.
