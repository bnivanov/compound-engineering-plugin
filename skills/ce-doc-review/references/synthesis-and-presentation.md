...
**Cross-model returns count as independent personas here only when the return's top-level `independence_verified` is `true`.** A return with `false` or a missing flag remains useful attributed reviewer evidence, but it cannot trigger anchor promotion or be described as separate-model corroboration. It still merges in 3.3 under the one-fix test like any other finding -- merging and corroboration are separate questions. On OMP the receipt always records `independence_verified: false`, so the read is attributed evidence, never separate-model corroboration.

When the cross-model judgment pass ran (see `references/cross-model-review.md`), each peer return enters synthesis as a reviewer named `<reviewer-name>-omp` (e.g. `adversarial-omp`). On OMP the receipt always records `independence_verified: false`, so agreement between such a `<reviewer-name>-omp` return and its in-process twin (`<reviewer-name>`) never promotes: it merges in 3.3 under the one-fix test like any other finding and stays attributed reviewer evidence. **The whole-document sweep** (`whole-doc-omp`) has **no in-process twin**, so its findings reach the output only when 3.3 merged them with an in-process reviewer's; they never promote either. **Corroboration only, never apply authority:** a peer-only finding is never silently applied as `safe_auto` (see the cross-model peer cap in 3.6 and the safeguard in 3.7); it caps at `gated_auto` unless an in-process reviewer independently corroborates it.
