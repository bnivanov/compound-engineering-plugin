...
**Scoped standards authority.** The local `project-standards` review and synthesis are the sole owners of scoped-rule coverage; the independent reviewer-agent return supplies candidate evidence. A reviewer-agent candidate enters the final report only when it is compatible with every applicable scoped rule established locally; reject it otherwise. A replacement candidate requires independent local evidence.
...
3. **Restore mechanics.** After semantic reconciliation, direct-dependency reclassification, or settlement reconciliation, rerun the helper with every reconciled candidate, including unchanged primary and pre-existing candidates plus remaining suppressed candidates. Omit candidates discarded during settlement reconciliation. It enforces the quote-the-line gate, discrete confidence anchors, exact dedup, independent-agreement promotion, conservative routing, pre-existing partition, confidence gate, deterministic sort, and stable `#` numbering. Agreement raises the anchor one step only when the merged group includes an `adversarial-omp` reviewer-agent return that records `independence_verified: true` from host/backend attestation; in-process reviewers share one serving model, so their agreement is recorded in `reviewers` but never raises confidence, and `fast-pass` never counts at all. That receipt requires a known serving family different from the session family, supplied by host/backend attestation, never reviewer prose. Reviewer-agent findings never carry apply authority.

**Quorum consensus dedup (synthesis judgment, before restore mechanics).** Merging two differently worded findings that describe the same defect and fix path is model judgment owned by step 1, never helper deduplication: the helper's exact-fingerprint rule stays unchanged. When you merge such a consensus candidate, list every contributing reviewer in its `reviewers`, and treat the merge as corroborated only when the source-detail map attributes those contributors to two distinct independent reviewer launches — a self-asserted attribution or a receipt from the same session family is not a quorum.

**Bounded refutation pass (consensus merges only).** When a differently worded consensus merge is carried as corroborated by two independent reviewer identities, run one bounded refutation pass before restore mechanics: one read-only re-inspection of each contributor's cited evidence, plus at most the one restore-mechanics helper rerun — never a second validator batch, extra reviewer launches, or open research. If the pass refutes the shared claim, drop the finding and record the refutation and its reason in Coverage; if it stands, name the pass in Coverage (trigger, contributing identities, outcome).
...
1. Skip a validator only when the finding has `first_evidence` and both an ordinary reviewer plus an `adversarial-omp` reviewer whose artifact records `independence_verified:true` from host/backend attestation. Same-model or unverified corroboration never licenses this shortcut. Record the skip count in Coverage.
...
Each object in `findings` uses the merged finding fields: `#`, `title`, `severity`, `file`, `line`, `confidence`, `autofix_class`, `owner`, `requires_verification`, `pre_existing`, `suggested_fix`, `first_evidence`, `why_it_matters`, `evidence`, `reviewers`, `independent_reviewers`. Each object in `learnings` is a Known Pattern note: `type` (`known_pattern`), `title`, `citation` (a `<root>/solutions/` path or `(pack: <id>, <path within the pack>)`), and `note` (one line on how it bears on the change); contradicted pack rules are findings, never `learnings` entries. `coverage.compound_packs` is present whenever the run had packs to resolve (Stage 1b's `declared_packs` true, or a declaration found at dispatch) or the scope is remote — where `declared_packs` is null and nothing resolves — and it carries `status`, `roots`, and the resolver's `warnings` and `errors` arrays verbatim, so a consumer can tell a declared pack that loaded from one that failed or was skipped. `status: resolved` means every declared pack loaded; `status: unresolved` means at least one did not — the `errors` lines say why, and the review must not read as pack-clean; `status: skipped-remote` means remote scope resolved nothing, because the local config is not the reviewed tree's config. `roots` lists the packs that did load, as ids (strings — never the resolver's absolute `dir` paths). Omit the key only when the scope is local and nothing declared packs; remote scope always carries it with `status: skipped-remote`. The helper derives `independent_reviewers`; synthesis may preserve or union that list but must not infer it from `reviewers`.

A finding Stage 5b left unresolved, or confirmed with an unmeasured-incidence reason, also carries `validation_status` and `validation_reason`, and carries `protected_subject` when one applies.
...
- `validator-verdicts.json`: the validator batch's verdicts (the on-disk fact)
