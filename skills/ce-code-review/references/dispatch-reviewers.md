...
If launched reviewers have no reliable blocking collection path, stop all launched agents and take the mode's failure path instead of waiting for notifications, emitting progress, or synthesizing a partial roster. In `mode:agent`, emit only `{"status":"failed","reason":"<one sentence>"}`. A host-specific collector such as a task-output tool is usable only when its live contract shows that it accepts the launch identifier, blocks until terminal, and returns the terminal outcome; do not infer those properties from the tool name.

This foreground batch is **not** the forbidden pattern. What is banned is turning local review into a *detached* delegate the orchestrator must poll: a background bash/CLI invocation plus foreground `sleep`/status-file loops, or scheduled wakeups and "still waiting" turns. The independent reviewer uses foreground `task` collection under the same rule; there is no separate background job. Harness blocking collection is allowed. Never insert shell no-ops (`echo waiting`, `noop`, `yield turn`, `end turn`, `true`, or sleeps), detached status polls, scheduled wakeups, or user-facing "still waiting" turns to await reviewers.
