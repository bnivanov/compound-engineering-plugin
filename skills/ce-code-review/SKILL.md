...
5. **Stage 3d.** When adversarial is selected for a local reviewed tree, run the fixed-`omp` separate read that `references/cross-model-review.md` defines, **before any local persona dispatch**. Invoking this skill is itself the authorization for the fixed `omp` route, once you have made the required disclosure that reviewed code leaves the machine to that route. Do not ask the user to confirm a second time, and do not skip the separate read because the user did not repeat that authorization. An explicit user prohibition on external review overrides it, as does a checkout that sets `cross_model_review_mode: off` with no live opt-in; both are resolved before you bind a route. This pass's only skip key is `cross_model_review_mode`. Missing files or unset keys take the default auto route; they are not a skip. A started separate read replaces the local adversarial persona at this stage, and only a real failure to scope, reach, authenticate, or start it leaves the local fallback in the roster; a later stage may still restore the local reviewer under the conditions that reference states.
