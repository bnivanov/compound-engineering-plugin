...
- **Fresh-context check.** Each cycle ends with one convergence check dispatched as a fresh native worker (per `references/execution-strategy.md` — no implementation-unit transcript): it reads the plan from disk (Product Contract, Implementation Units, Verification Contract, Definition of Done), the current task list, and — when the project carries `constitution.md` at its root — that per-project constitution file (absence is normal), then inspects the actual tree and reports either **converged** or named gaps. This loop's context supplies evidence pointers to the check, never the verdict; a convergence verdict this loop awards itself does not end the cycle. The check is never routed through `ce-verify`: ce-verify owns drive-infrastructure generate/refresh, never QA judgment.
