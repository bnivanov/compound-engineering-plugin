...
This loop owns unit ordering, evidence selection, actual-scope inspection, authoritative verification, and incremental canonical commits for the native flow in `references/execution-strategy.md`. Do not mark the task complete until the host orchestrator records the canonical commit. A preserved or restoration-blocked unit stops this loop before fallback, retry, or the next unit.
...
- Envelope enforcement point (MUST): evidence recorded by this loop is input, never verdict — the PASS/BLOCK decision evaluates only at orchestrator-envelope emission via `scripts/envelope-gate.py` (see `references/return-to-caller.md`). A worker return, including this loop's own completion marks, never satisfies the gate; a per-worker hook or self-certified complete promotes unverified work.
...
A complete task list is not yet a converged implementation. When the last task in the list completes, judge convergence against the plan before the run's tail — standalone: before the Phase 3-4 transition; Return-to-Caller Mode: inside per-unit fix-before-next (mode placement below). Convergence is never certified by the context that did the implementing — self-certification is the trust failure this gate exists to prevent.

- **Fresh-context check.** Each cycle ends with one convergence check dispatched as a fresh native worker (per `references/execution-strategy.md` — no implementation-unit transcript): it reads the plan from disk (Product Contract, Implementation Units, Verification Contract, Definition of Done), the current task list, and — when the project carries `constitution.md` at its root — that per-project constitution file (absence is normal), then inspects the actual tree and reports either **converged** or named gaps. A verdict counts only when it carries the plan artifacts the check was made against and the per-cycle outcome it rests on. A bare `converged` without that content does not converge, and the gate does not end the cycle on it. This loop's context supplies evidence pointers to the check, never the verdict; a convergence verdict this loop awards itself does not end the cycle. A verdict exists only when a fresh context rendered it. When fresh dispatch is unavailable, this loop does not judge its own work inline: the gate reaches no verdict, and the cycle hands back (or stops `blocked`) with the gaps named and the fresh convergence context still owed. The inline fallback that `references/execution-strategy.md` grants implementation units never covers the convergence check. No state of the dispatch turns the implementing context into the judge. The check is never routed through `ce-verify`: ce-verify owns drive-infrastructure generate/refresh, never QA judgment.
- **Scope-guarded appends.** A reported gap becomes a new task appended to the task list only when it is inside the plan's Scope Boundaries, and it follows the same unit discipline (bounded packet, declared files, unit-scoped incremental commits) — no scope-creep commit. A gap outside the plan's Scope Boundaries is never silently appended and never folded into an in-scope task to dodge the check: the cycle stops `blocked` for re-planning with that gap named.
- **Bounded.** The implementation cycle — execute, check, append the remaining work as new tasks, execute again — runs by default at most **3 cycles**, matching the shipping tail's bounded-repair precedent (a plan or the invoking caller may set a different bound explicitly; there is no unbounded mode). An override is itself a stated finite cycle count: declared before the first cycle runs, and never raised while the run's cycles continue. A bound re-raised mid-run is the unbounded mode the section disclaims. At the budget with gaps outstanding, stop the run `blocked` with a recovery path naming the remaining gaps and the route that clears them (re-dispatch the implementation with the same plan, or re-plan) — never hold indefinitely, never drop the gaps silently.
- **Mode placement.** Standalone, this gate runs here and a `blocked` outcome stops the shipping transition; a converged report ends the loop and the run proceeds to its tail. In Return-to-Caller Mode the loop lives inside per-unit fix-before-next (`references/execution-strategy.md`): each unit's fix-before-next cycles under the same fresh-context check and the same default bound — the check judges that unit against its unit packet — and a not-converged outcome emits `status: blocked` with the gaps as blockers and a recovery path. Outer re-dispatch belongs to the caller: the caller owns any run-level acceptance check, and `references/return-to-caller.md` remains the only emission authority.
