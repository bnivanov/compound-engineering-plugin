...
**Conflict gate — surface it when it would change a product decision.** Apply the kernel Overloaded-terms condition at this step. Fire only when overload or a clash with existing `CONCEPTS.md` or verified code would change a requirement or decision. Challenge with a concrete contrasting edge case and a code cross-check, and put that conflict to them before treating their wording as settled. The same live challenge applies when a claim about how the system works conflicts with the grounding dossier. Do not create `CONCEPTS.md`. Glossary writes still wait until after the plan.
